...
class Device extends EventEmitter {
  constructor(devicePath, nameMap, valueMaps) {
    ...
    device.on('open', () => {
      ...
      if (detectedIdMaps) {
        if (detectedIdMaps.names && !nameMap) {
          this.nameMap = detectedIdMaps.names;
        }
    ...
    if (typeof nameMap === 'string') {
      nameMap = shush(path.resolve(__dirname, '../id-maps', nameMap)).names;
    }
    if (typeof valueMaps === 'string') {
      valueMaps = shush(path.resolve(__dirname, '../id-maps', valueMaps)).values;
    }
  ...
  mapAndEmit(data) {
    ...
    if (this.nameMap[id] === null) {
      // don't report
      return;
    }
    const mappedId = this.nameMap[id] || id;
    const valueMap = this.valueMaps[mappedId];
    const mappedValue = valueMap ? valueMap(value) : value;
    this.emit(mappedType, mappedId, mappedValue);
  }
